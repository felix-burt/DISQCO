...
def find_gain(hypergraph,node,destination,assignment,num_partitions,costs,log=False):
    ...
    for edge in edges:
        ...
        # root_counts = hypergraph.get_hyperedge_attribute(edge,'root_counts')
        # root_config = hypergraph.get_hyperedge_attribute(edge,'root_config')
        # rec_counts = hypergraph.get_hyperedge_attribute(edge,'rec_counts')
        # rec_config = hypergraph.get_hyperedge_attribute(edge,'rec_config')
        # if node in root_set:
        #     root_counts_new, source_root = update_counts(root_counts,node,destination,assignment)
        #     root_config_new = update_config(root_config,root_counts_new,source_root,destination)
        #     rec_counts_new = tuple(copy.deepcopy(list(rec_counts)))
        #     rec_config_new = tuple(copy.deepcopy(list(rec_config)))
        # elif node in rec_set:
        #     rec_counts_new, source_rec = update_counts(rec_counts,node,destination,assignment)
        #     rec_config_new = update_config(rec_config,rec_counts_new,source_rec,destination)
        #     root_counts_new = tuple(copy.deepcopy(list(root_counts)))
        #     root_config_new = tuple(copy.deepcopy(list(root_config)))
        # config_new = get_full_config(root_config_new,rec_config_new)
        # if log:
        #     print("Full config new",config_new)
        # cost2comp = costs[config_new]
        # cost2 = cost2comp
        gain += cost2 - cost1
    if log:
...
def check_array(hypergraph,array,assignment,num_qubits,num_partitions,costs,lock_dict,buckets, log_strings):
    corrected_array = find_all_gains(hypergraph,assignment,num_qubits, num_partitions, costs)
    for i in range(len(array)):
        for j in range(len(array[i])):
